document.addEventListener('DOMContentLoaded', function () {
  ...
  if ('IntersectionObserver' in window) {
    const observerOptions = {
      root: null,
      rootMargin: '0px 0px -' + Math.round(ANIMATION_OFFSET * 100) + '% 0px',
      threshold: 0
    };
    ...
    function observeElements() {
      getAnimatableElements().forEach(function (el) {
        if (!el.classList.contains(ANIMATED_CLASS)) {
    ...
    const mutationObserver = new MutationObserver(function (mutations) {
      mutations.forEach(function (mutation) {
        mutation.addedNodes.forEach(function (node) {
          ...
          if (
            node.hasAttribute(ANIMATION_ATTRIBUTE) ||
            node.classList.contains('fade-in') ||
            node.classList.contains('slide-up') ||
            node.classList.contains('slide-in') ||
            node.classList.contains('reveal')
          ) {
            if (!node.classList.contains(ANIMATED_CLASS)) {
          ...
          children.forEach(function (child) {
            if (!child.classList.contains(ANIMATED_CLASS)) {
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '44de264f-9408-42b1-8820-2ddf9b6599fc',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '44de264f-9408-42b1-8820-2ddf9b6599fc';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '44de264f-9408-42b1-8820-2ddf9b6599fc';
